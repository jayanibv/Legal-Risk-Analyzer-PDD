...
class TestEdgeCases:
    ...
    def test_tc111_analyze_very_long_text(self):
        """TC111: Analyzing a very long text (>5000 chars) does not crash the server."""
        long_text = ("This agreement shall be binding. " * 200)  # ~6600 chars
    ...
    def test_tc113_analyze_whitespace_only_text(self):
        """TC113: Analyzing whitespace-only text is handled gracefully."""
        r = requests.post(f"{BASE_URL}/analyze",
    ...
    def test_tc114_analyze_unicode_text(self):
        """TC114: Analyzing text with Unicode characters does not crash server."""
        ...
        assert r.status_code in (200, 400, 422, 500), \
            f"Unexpected status for Unicode text: {r.status_code}"
    ...
    def test_tc115_signup_with_special_chars_in_name(self):
        ...
        assert r.status_code in (200, 400, 422), \
            f"Unexpected status for special char name: {r.status_code}"
