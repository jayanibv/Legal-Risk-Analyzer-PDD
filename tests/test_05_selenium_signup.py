...
class TestSignupPage:
    ...
    def test_tc072_login_link_on_signup_page(self, driver):
        ...
        body = driver.find_element(By.TAG_NAME, "body").text
        if "Login" not in body and "log in" not in body.lower() and "Sign In" not in body:
            time.sleep(3)
            body = driver.find_element(By.TAG_NAME, "body").text
        assert "Login" in body or "log in" in body.lower() or "Sign In" in body, \
            f"Login link not found on signup page. Body: {body[:300]}"
